...
class LoRATrainer(Trainer):
    """Custom Trainer with metrics logging - records both train and eval loss separately"""
    
    def __init__(self, *args, **kwargs):
        kwargs.pop("compute_metrics", None)
        super().__init__(*args, **kwargs)
        self.train_metrics = []  # (step, train_loss, learning_rate)
        self.eval_metrics = []   # (step, eval_loss, perplexity)
    ...
    @property
    def metrics(self):
        """Combine train and eval metrics for plotting"""
        ...
        train_dict = {m["step"]: m for m in self.train_metrics}
        eval_dict = {m["step"]: m for m in self.eval_metrics}
        
        for step in result["step"]:
            if step in train_dict:
                result["train_loss"].append(train_dict[step]["train_loss"])
                result["learning_rate"].append(train_dict[step]["learning_rate"])
            else:
                result["train_loss"].append(None)
                result["learning_rate"].append(None)
            
            if step in eval_dict:
                result["eval_loss"].append(eval_dict[step]["eval_loss"])
                result["perplexity"].append(eval_dict[step]["perplexity"])
            else:
                result["eval_loss"].append(None)
                result["perplexity"].append(None)
